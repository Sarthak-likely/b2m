...
- ⚡ **Multi-threaded processing** - Uses all CPU cores via pthreads
- 🎹 **Customizable note range** - Map bytes to any MIDI note range
- 🎛️ **Note duration control** - Set individual note length (1-127 ticks)
- ⏱️ **Time compression** - Cram/extend entire song to exact duration
- 🚀 **ARM64/ARM32 optimized** - NEON SIMD when available, scalar fallback
- 📦 **Zero dependencies** - Pure C, POSIX, no external libraries
- 🔋 **Memory efficient** - mmap() zero-copy file reading
...
|--------|-----|--------------|--------|
| Vivo Y91i | MediaTek Helio P22 | ARM64 (Cortex-A53) | ✅ Tested |
| Vivo Y15s | MediaTek Helio P35 | ARM32 (Cortex-A53) | ⏳ Pending |
| Generic x86_64 | - | x86_64 | ❌ Untested |

**Want to test on your device?** 
- ✅ If it works: Open a closed issue with "Tested: [device]" 
- ❌ If it fails: Open an issue with full error log and device info
...
<div align="center">
  <sub>Built with ⚡ for ARM64 Termux</sub>
</div>
...
Warning: Files not uploaded yet